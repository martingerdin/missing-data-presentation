...
get_results <- function(x, y, data) {
    ...
    pv <- paste0(v[, 1], " (", v[, 2], "-", v[, 3], ")")
    return(pv)
}
models.data <- table.data
models.data$Died <- as.numeric(models.data$Died == "Yes")
variables <- setNames(nm = c("Age", "Sex", "SBP", "RR", "GCS"))
unadjusted.results <- unlist(lapply(variables, get_results, y = "Died", data = models.data))
adjusted.results <- get_results(variables, "Died", data = models.data)
combined <- as.data.frame(cbind(unadjusted.results, adjusted.results))
names(combined) <- c("Unadjusted OR (95% CI)", "Adjusted OR (95% CI)")
```

```{r, echo=FALSE, results="asis"}
knitr::kable(combined, caption = "Unadjusted and adjusted associations between variables and mortality")
```
